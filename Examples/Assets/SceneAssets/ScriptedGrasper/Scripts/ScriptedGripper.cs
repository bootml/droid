...
using SceneAssets.ScriptedGrasper.Grasps;
using SceneAssets.ScriptedGrasper.Navigation;
using SceneAssets.ScriptedGrasper.Utilities;
using UnityEngine;

namespace SceneAssets.ScriptedGrasper.Scripts {
    ...
    void Ab(GameObject child_game_object, GraspableObject other_maybe_graspable) {
      if (other_maybe_graspable) {
        if (child_game_object == this._grab_region.gameObject
          ...
          if (this._debugging)
            print($"Target {other_maybe_graspable.name} is inside region");
        ...
        if (child_game_object == this._begin_grab_region.gameObject
          ...
          if (this._debugging)
            print($"Picking up target {other_maybe_graspable.name}");
    ...
    void OnCollisionStayChild(GameObject child_game_object, Collision collision) {
      ...
      if (other_maybe_graspable) {
        if (child_game_object == this._claw_1.gameObject
            && other_maybe_graspable.gameObject == this._target_game_object.gameObject) {
          if (this._debugging) {
            print($"Target {other_maybe_graspable.name} is touching {child_game_object.name}");
          }
